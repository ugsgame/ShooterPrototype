...
void AShooterWeapon::OnStartFire()
{
	NotifyBPOnStartFire();
}

void AShooterWeapon::OnStopFire()
{
	NotifyBPOnStopFire();
}
...
void AShooterWeapon::AttachMeshToPawn()
{
	if (MyPawn)
	{
		...
		else
		{
			...
			UseWeaponMesh->SetOwnerNoSee(!MyPawn->IsLocallyControlled());
		}

	}
}

void AShooterWeapon::AttachMeshToInventory()
{
	if (MyPawn)
	{
		// Remove and hide both first and third person meshes
		...
		// For locally controller players we attach both weapons and let the bOnlyOwnerSee, bOwnerNoSee flags deal with visibility.
		FName AttachPoint = MyPawn->GetInventoryWeaponPoint(WeaponConfig.InvetoryWeaponPointIndex);
		if (MyPawn->IsFirstPerson())
		{
			if (MyPawn->IsLocallyControlled() == true)
			{
...
void AShooterWeapon::OnPlayerTypeChanged(EShooterPlayerType PlayerType)
{
	//Refresh
	AttachMeshToPawn();
}
...
FVector AShooterWeapon::GetAdjustedAim() const
{
	...
	if (PlayerController)
	{
		if (GetPawnOwner() && GetPawnOwner()->GetPlayerType() == EShooterPlayerType::TSS)
		{
...
float AShooterWeapon::GetFireLoudness() const
{
	return FireLoudness;
}
