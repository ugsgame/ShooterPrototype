...
struct FWeaponData
{
	...
	UPROPERTY(EditDefaultsOnly, Category = WeaponEquip)
	FName WeaponAttachPoint;

	UPROPERTY(EditDefaultsOnly, Category = WeaponEquip)
	int32 InvetoryWeaponPointIndex;
	...
	FWeaponData()
	{
		...
		WeaponAttachPoint = FName(TEXT("WeaponPoint"));
		InvetoryWeaponPointIndex = 0;
...
class SHOOTERGAME_API AShooterWeapon : public AActor
{
	...
	virtual EShooterWeaponType GetWeaponType() const
	{
	...
	{
		return WeaponConfig.WeaponAttachPoint;
	}

	UFUNCTION(BlueprintCallable, Category = Weapon)
		float GetWeaponAttackRange()
	{
		return WeaponConfig.AttackRange;
	}

	UFUNCTION(BlueprintCallable, Category = Weapon)
		float GetWeaponAlarmRange()
	{
		return WeaponConfig.AlarmRange;
	}

	UFUNCTION(BlueprintCallable, Category = Weapon)
		float GetWeaponRetreatRange()
	{
		return WeaponConfig.RetreatRange;
	}

	virtual void OnPlayerTypeChanged(EShooterPlayerType PlayerType);
	...
	/** gets the Loundess */
	float GetFireLoudness() const;
protected:

	UFUNCTION(BlueprintImplementableEvent)
	void NotifyBPOnStartFire();

	UFUNCTION(BlueprintImplementableEvent)
	void NotifyBPOnStopFire();
...
protected:
	...
	/** attaches weapon mesh to pawn's inventory */
	virtual void AttachMeshToInventory();
