...
#if PLATFORM_SWITCH
#	define LEADERBOARDS_SUPPORTED 1
#else
#	define LEADERBOARDS_SUPPORTED 1
#endif
...
void FShooterMainMenu::OnUserCanPlayOnlineQuickMatch(const FUniqueNetId& UserId, EUserPrivileges::Type Privilege, uint32 PrivilegeResults)
{
	...
	if (PrivilegeResults == (uint32)IOnlineIdentity::EPrivilegeResults::NoFailures)
	{
		...
		SplitScreenLobbyWidget->SetIsLAN(false);
...
#if PLATFORM_SWITCH
	GameInstance->UpdateUsingMultiplayerFeatures(true);
#endif
...
void FShooterMainMenu::OnSplitScreenSelected()
{
	...
	SplitScreenLobbyWidget->SetIsLAN(bIsLanMatch);
...
{	
	...
	{
		FString const StartURL = FString::Printf(TEXT("/Game/Prototype/Maps/%s?game=%s%s%s?%s=%d%s"), *GetMapName(), *GameType, GameInstance->GetIsOnline() ? TEXT("?listen") : TEXT(""), bIsLanMatch ? TEXT("?bIsLanMatch") : TEXT(""), *AShooterGameMode::GetBotsCountOptionName(), BotsCountOpt, bIsRecordingDemo ? TEXT("?DemoRec") : TEXT("") );
...
#if PLATFORM_SWITCH
		GameInstance->UpdateUsingMultiplayerFeatures(!bIsLanMatch);
#endif
