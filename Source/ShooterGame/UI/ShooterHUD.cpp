...
AShooterHUD::AShooterHUD(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	static ConstructorHelpers::FObjectFinder<UTexture2D> HitTextureOb(TEXT("/Game/Prototype/UI/HUD/HitIndicator"));
	static ConstructorHelpers::FObjectFinder<UTexture2D> HUDMainTextureOb(TEXT("/Game/Prototype/UI/HUD/HUDMain"));
	static ConstructorHelpers::FObjectFinder<UTexture2D> HUDAssets02TextureOb(TEXT("/Game/Prototype/UI/HUD/HUDAssets02"));
	static ConstructorHelpers::FObjectFinder<UTexture2D> LowHealthOverlayTextureOb(TEXT("/Game/Prototype/UI/HUD/LowHealthOverlay"));

	// Fonts are not included in dedicated server builds.
	#if !UE_SERVER
	{
		static ConstructorHelpers::FObjectFinder<UFont> BigFontOb(TEXT("/Game/Prototype/UI/HUD/Roboto51"));
		static ConstructorHelpers::FObjectFinder<UFont> NormalFontOb(TEXT("/Game/Prototype/UI/HUD/Roboto18"));
...
void AShooterHUD::DrawCrosshair()
{
	...
	if (PCOwner)
	{
		...
		{
			const float SpreadMulti = Pawn->GetPlayerType() == EShooterPlayerType::TSS?30:300;
			...
			if (Pawn->GetPlayerType() == EShooterPlayerType::TSS)
			{
				PCOwner->GetMousePosition(CenterX, CenterY);
			}
