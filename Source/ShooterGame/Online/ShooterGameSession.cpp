...
void AShooterGameSession::HandleMatchHasStarted()
{
	...
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid())
		{
			UE_LOG(LogOnlineGame, Log, TEXT("Starting session %s on server"), *FName(GameSessionName).ToString());
...
void AShooterGameSession::HandleMatchHasEnded()
{
	...
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid())
		{
			...
			UE_LOG(LogOnlineGame, Log, TEXT("Ending session %s on server"), *FName(GameSessionName).ToString() );
