...
void UShooterGameInstance::CleanupSessionOnReturnToMenu()
{
	...
	if ( Sessions.IsValid() )
	{
		...
		{
			UE_LOG(LogOnline, Log, TEXT("Destroying session %s on return to main menu"), *FName(GameSessionName).ToString() );				
...
bool UShooterGameInstance::HostGame(ULocalPlayer* LocalPlayer, const FString& GameType, const FString& InTravelURL)
{
	...
	if (GameSession)
	{
		...
		const FString& MapNameSubStr = "/Game/Prototype/Maps/";
		...
		if (GameSession->HostSession(LocalPlayer->GetPreferredUniqueNetId().GetUniqueNetId(), GameSessionName, GameType, MapName, bIsLanMatch, true, AShooterGameSession::DEFAULT_NUM_PLAYERS))
		{
...
bool UShooterGameInstance::FindSessions(ULocalPlayer* PlayerOwner, bool bIsDedicatedServer, bool bFindLAN)
{
	...
	if (PlayerOwner)
	{
		...
		if (GameSession)
		{
			...
			GameSession->FindSessions(PlayerOwner->GetPreferredUniqueNetId().GetUniqueNetId(), GameSessionName, bFindLAN, !bIsDedicatedServer);
...
void UShooterGameInstance::BeginHostingQuickMatch()
{
	...
	// Travel to the specified match URL
	GetWorld()->ServerTravel(TEXT("/Game/Prototype/Maps/Highrise?game=TDM?listen"));	
}
